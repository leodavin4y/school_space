...
class AdminController extends BaseApiController {
    ...
    public function userSetBalance(int $userId, ValidatorInterface $validator, UsersRepository $usersRep)
    {
        ...
        try {
            $letscoverBalance = Letscover::getActivity($user->getUserId());

            if (!$letscoverBalance) throw new \Exception('Failed to fetch LetscoverAPI points');

            if ($letscoverBalance->points > $params['balance']) {
                Letscover::subBalance($user->getUserId(), $letscoverBalance->points - $params['balance']);
            } else {
                Letscover::addBalance($user->getUserId(), $params['balance'] - $letscoverBalance->points);
            }
